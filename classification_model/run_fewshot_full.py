"""
run_fewshot_full.py — resumable, shardable few-shot grading runner for all
200 essays (15 reference + 185 new), using the validated 4-exemplar
few-shot block (correlation 0.048 -> 0.309 on the 17-essay test).

Same resumable/sharding pattern as every other runner in this project —
safe to stop and re-run, splittable across multiple Groq accounts. Given
few-shot roughly doubles-to-triples token cost per call, expect lower
daily throughput per account than earlier (non-few-shot) runs — the
resumable design handles this automatically regardless of exactly where
the daily cap lands.

Usage (run from repo root, GROQ_API_KEY set):

    Solo:
        python -m classification_model.run_fewshot_full [max_this_run]
...
def progress_path_for(shard_id: int | None) -> Path:
    ...
    return PROGRESS_DIR / f"fewshot_full_progress{suffix}.csv"
