"""
run_batch2_ai_grading.py — resumable, shardable AI-grading runner for the
185 new essays that now have real professor-consensus scores (from
consolidated_singlesheet_v2_AI_graded.xlsx / whatever it gets renamed to).

Same resumable/sharding pattern as run_dress_training_batch.py — safe to
stop and re-run, and supports splitting across multiple Groq accounts.

Usage (run from repo root, GROQ_API_KEY set):

    Solo:
        python -m classification_model.run_batch2_ai_grading [max_this_run]

    Sharded across N accounts (sizes must sum to 185):
        python -m classification_model.run_batch2_ai_grading [max_this_run] \
            --shard-sizes 27,27,27,26,26,26,26 --shard-id 0
        ...(one invocation per shard_id, 0-indexed)...

PROFESSOR_SCORES_XLSX env var overrides the default path to the filled-in
professor scores workbook.
"""
...
from .dress_pipeline import MAX_TOTAL_POINTS, build_dress_rubric_criteria_json
from .professor_ground_truth import load_professor_consensus
from .build_professor_batch2_workbooks import select_new_185

DEFAULT_XLSX_PATH = r"C:\Users\ASUS\Downloads\consolidated_singlesheet_v2_AI_graded.xlsx"
...
def progress_path_for(shard_id: int | None) -> Path:
    ...
    return PROGRESS_DIR / f"batch2_ai_progress{suffix}.csv"
...
def main():
    ...
    print("Loading the 185 new essays and professor-consensus scores...")
    essays = select_new_185()  # already has essay_id, question_prompt, essay_text
